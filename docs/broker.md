...
4. Вкладка **КП** — UI-заглушка (реальных данных КП пока нет).

### Текущие дела — создание

1. Модалка: поля + **Ответственный** под «Переговоры» (disabled без помещения; список при открытии селектора).
