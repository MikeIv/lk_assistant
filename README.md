...
**Auth и роли** — JWT-сессия; UI-роль из access JWT: [`useCabinetRole`](app/composables/useCabinetRole.ts) (`admin` / `user`). Подробнее: [`docs/auth.md`](docs/auth.md).
...
| [docs/broker/](docs/broker/)                 | Календарь, задачи, текущие дела       |
