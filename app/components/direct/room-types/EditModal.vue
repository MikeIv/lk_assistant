...
const props = defineProps<{
  ...
  submitFn: (id: number, payload: RoomTypeCreatePayload) => Promise<RoomTypeCreateResult>
  deleteFn: (id: number) => Promise<RoomTypeDeleteResult>
}>()
...
watch(
  () => [open.value, props.roomType] as const,
  ([isOpen, roomType]) => {
    if (isOpen && roomType) {
      resetForm({
        values: {
...
const onSubmit = handleSubmit(async () => {
  ...
  try {
    const result = await props.submitFn(props.roomType.id, toPayload())
...
async function handleDeleteConfirm() {
  ...
  try {
    const result = await props.deleteFn(props.roomType.id)
...
<template>
  <Teleport to="body">
    <div v-if="open && roomType" :class="$style.overlay" @mousedown.self="close">
      <form
        :class="$style.dialog"
        role="dialog"
        aria-modal="true"
        aria-labelledby="room-types-edit-title"
        @submit.prevent="onSubmit"
      >
        <div :class="$style.header">
          <h3 id="room-types-edit-title" :class="$style.title">Редактирование типа помещения</h3>
          <button
            type="button"
        ...
        <div v-if="isDeleteConfirmOpen" :class="$style.deleteConfirm">
          <p :class="$style.deleteConfirmText">Удалить тип помещения «{{ roomType.name }}»?</p>
...
.inputWrapError {
  animation: room-types-field-error-blink 1.2s ease-in-out 2;
...
@keyframes room-types-field-error-blink {
  0%,
  100% {
    box-shadow: 0 0 0 2px var(--fs-color-error);
  }
