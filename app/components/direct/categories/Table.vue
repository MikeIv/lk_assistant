...
import { CATEGORIES_PER_PAGE_OPTIONS } from '#shared/utils/categoriesTable'
...
const emit = defineEmits<{
  ...
  sortChange: [key: CategorySortKey]
  rowClick: [item: Category]
  create: []
}>()
...
function sortAriaLabel(key: CategorySortKey, label: string): string {
  ...
  <div :class="$style.root">
    ...
    <footer :class="$style.footer">
      ...
      <div :class="$style.perPage">
        <label :class="$style.perPageLabel" for="categories-per-page">на страницу</label>
        <select
        ...
        >
          <option v-for="option in CATEGORIES_PER_PAGE_OPTIONS" :key="option" :value="String(option)">
            {{ option }}
          </option>
        </select>
      </div>
...
.table {
  ...
  --categories-header-bg: var(--fs-figma-achromatic-middle-gray);
  --categories-header-fg: var(--fs-figma-achromatic-white);
  --categories-header-divider: rgb(255 255 255 / 0.12);
  --categories-header-divider-soft: rgb(255 255 255 / 0.08);
}
...
.thead {
  ...
  top: 0;
  z-index: 2;
  background-color: var(--categories-header-bg);
}

.th {
  padding: rem(14) rem(12);
  border-bottom: 1px solid var(--categories-header-divider);
  border-right: 1px solid var(--categories-header-divider-soft);
  background-color: var(--categories-header-bg);
...
.sortButton {
  ...
  font-weight: 600;
  line-height: 1.2;
  color: var(--categories-header-fg);
  white-space: nowrap;
