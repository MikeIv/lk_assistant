...
import { NEGOTIATION_STATUSES_PER_PAGE_OPTIONS } from '#shared/utils/negotiationStatusesTable'

const props = defineProps<{
  items: NegotiationStatus[]
  pagination: NegotiationStatusesPagination
  searchQuery: string
...
const emit = defineEmits<{
  ...
  sortChange: [key: NegotiationStatusSortKey]
  rowClick: [item: NegotiationStatus]
  create: []
}>()
...
function sortAriaLabel(key: NegotiationStatusSortKey, label: string): string {
  ...
  <div :class="$style.root">
    ...
    <footer :class="$style.footer">
      ...
      <div :class="$style.perPage">
        <label :class="$style.perPageLabel" for="negotiation-statuses-per-page">на страницу</label>
        <select
          id="negotiation-statuses-per-page"
          :class="$style.perPageSelect"
          :value="String(perPage)"
          @change="handlePerPageChange"
        >
          <option
            v-for="option in NEGOTIATION_STATUSES_PER_PAGE_OPTIONS"
            :key="option"
            :value="String(option)"
          >
            {{ option }}
...
.table {
  ...
  --negotiation-statuses-header-bg: var(--fs-figma-achromatic-middle-gray);
  --negotiation-statuses-header-fg: var(--fs-figma-achromatic-white);
  --negotiation-statuses-header-divider: rgb(255 255 255 / 0.12);
  --negotiation-statuses-header-divider-soft: rgb(255 255 255 / 0.08);
}
...
.thead {
  ...
  top: 0;
  z-index: 2;
  background-color: var(--negotiation-statuses-header-bg);
}

.th {
  padding: rem(14) rem(12);
  border-bottom: 1px solid var(--negotiation-statuses-header-divider);
  border-right: 1px solid var(--negotiation-statuses-header-divider-soft);
  background-color: var(--negotiation-statuses-header-bg);
...
.sortButton {
  ...
  font-weight: 600;
  line-height: 1.2;
  color: var(--negotiation-statuses-header-fg);
  white-space: nowrap;
...
.spinner {
  ...
  border-radius: 50%;
  animation: negotiation-statuses-spin 0.8s linear infinite;
}
...
@keyframes negotiation-statuses-spin {
  to {
