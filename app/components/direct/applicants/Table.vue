...
import { APPLICANTS_PER_PAGE_OPTIONS } from '#shared/utils/applicantsTable'
...
const emit = defineEmits<{
  ...
  sortChange: [key: ApplicantSortKey]
  rowClick: [item: Applicant]
  create: []
}>()
...
function sortAriaLabel(key: ApplicantSortKey, label: string): string {
...
function categoryLabel(item: Applicant): string {
  return item.category_name ?? String(item.category_id)
}
</script>
  ...
  <div :class="$style.root">
    ...
    <footer :class="$style.footer">
      ...
      <div :class="$style.perPage">
        <label :class="$style.perPageLabel" for="applicants-per-page">на страницу</label>
        <select
          id="applicants-per-page"
          :class="$style.perPageSelect"
          :value="String(perPage)"
          @change="handlePerPageChange"
        >
          <option v-for="option in APPLICANTS_PER_PAGE_OPTIONS" :key="option" :value="String(option)">
            {{ option }}
          </option>
        </select>
      </div>
...
.table {
  ...
  --applicants-header-bg: var(--fs-figma-achromatic-middle-gray);
  --applicants-header-fg: var(--fs-figma-achromatic-white);
  --applicants-header-divider: rgb(255 255 255 / 0.12);
  --applicants-header-divider-soft: rgb(255 255 255 / 0.08);
}
...
.thead {
  ...
  top: 0;
  z-index: 2;
  background-color: var(--applicants-header-bg);
}

.th {
  padding: rem(14) rem(12);
  border-bottom: 1px solid var(--applicants-header-divider);
  border-right: 1px solid var(--applicants-header-divider-soft);
  background-color: var(--applicants-header-bg);
...
.sortButton {
  ...
  font-weight: 600;
  line-height: 1.2;
  color: var(--applicants-header-fg);
  white-space: nowrap;
...
.spinner {
  ...
  border-radius: 50%;
  animation: applicants-spin 0.8s linear infinite;
}
